...
data class SyncGroupStore(
    ...
    inline operator fun <reified T : Any> plusAssign(value: T?) =
        putByClassInstance(T::class, value)
    ...
    suspend fun <T : Any> await(timeout: Long, key: String): Result<T> {
        val entry = dict[key] as? T
        if (entry != null) return Result.success(entry)
    ...
    suspend fun <T : Any> awaitOrNull(timeout: Long, key: String): T? =
        await<T>(timeout, key).getOrNull()

    suspend inline fun <reified T : Any> await(timeout: Long): Result<T> {
    ...
    suspend inline fun <reified T : Any> awaitOrNull(timeout: Long): T? =
        await<T>(timeout).getOrNull()
